...
R-squared (R2) and the correlation coefficient (r) have the following
relationships:

The square of the correlation coefficient (r) is known as R-squared (R²). 
...
```{r}
# Construct a linear model using mileage per gallon (mpg) to estimate horsepower
# (hp)
...
#3. For this question, we are going to use BostonHousing dataset. The dataset is
...
every unit increase in ptratio.There will be a drop of 15 * 1493.67 = 22405.05.
...
d) d) Use the anova analysis and determine the order of importance of these four
variables
